...
def verify_local_e2e_safety(payload: object) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = []
    if isinstance(payload, dict):
        if payload.get("local_e2e_verification_only") is False:
            findings.append({"kind": "boundary", "match": "local_e2e_verification_only_false"})
    ...
    blocked_terms = ["raw provider payload", "real_account_id", "real_order_id", "paper-api.", "api.alpaca.", "https://sandbox", "alpaca_trade_api", "ib_insync", "tigeropen", "robin_stocks"]
...
def build_local_e2e_safety_summary() -> dict:
    status = get_local_e2e_status()
    safety = verify_local_e2e_safety(status)
