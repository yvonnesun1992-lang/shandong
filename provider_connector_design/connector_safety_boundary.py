...
from config.v5_provider_connector_design_config import get_connector_design_status
from provider_connector_design import boundary


def build_connector_safety_boundary() -> dict:
    summary = validate_connector_design_safety(get_connector_design_status())
    summary["checks"].extend(
        [
            {"check": "no broker SDK import", "status": "OK"},
            {"check": "no credentials loaded", "status": "OK"},
            {"check": "manual approval remains mandatory", "status": "OK"},
            {"check": "kill switch required before runtime", "status": "OK"},
        ]
    )
    return summary


def validate_connector_design_safety(payload: object | None = None) -> dict:
    state = payload or get_connector_design_status()
    text = json.dumps(state, default=str).lower()
    checks = []
    errors = []
    for key in ["connector_runtime_enabled", "sandbox_api_enabled", "account_read_enabled", "order_submission_enabled", "broker_connected", "real_money_enabled"]:
        value = state.get(key, False) if isinstance(state, dict) else False
        ok = value is False
        checks.append({"check": key, "status": "OK" if ok else "ERROR"})
        if not ok:
            errors.append({"check": key, "message": f"{key} must remain false in V5.21"})
    blocked_patterns = [
        ...
        r"real[_-]?order[_-]?ref\s*[:=]\s*[a-z0-9]",
        r"account[_-]?ref\s*[:=]\s*[a-z0-9]",
        r"sk-[a-z0-9]",
    ]
