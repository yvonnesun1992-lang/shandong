...
def validate_fault_safety(payload: dict | list | str) -> dict:
    ...
    for key in [
        ...
        if failed:
            errors.append(f"{key} must remain false in V5.24")
...
def build_fault_safety_summary() -> dict:
    return validate_fault_safety(boundary())
