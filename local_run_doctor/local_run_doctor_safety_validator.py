...
def validate_local_run_doctor_safety(payload: object) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = []
    if isinstance(payload, dict):
        if payload.get("local_run_doctor_only") is False:
            findings.append({"kind": "boundary", "match": "local_run_doctor_only_false"})
...
def build_local_run_doctor_safety_summary() -> dict:
    status = get_local_run_doctor_status()
    safety = validate_local_run_doctor_safety(status)
