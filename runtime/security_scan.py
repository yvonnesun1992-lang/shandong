...
def scan_product_home_outputs(payload: dict | list | str, report_path: str | Path | None = None) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = _scan_sandbox_sensitive_text(text)
    blocked_terms = [
        "api_key=demo",
    ...
    for term in blocked_terms + runtime_terms:
        if term in text:
            findings.append({"kind": "sensitive-pattern", "match": "blocked-product-home-output"})
            break
    if "http://" in text or "https://" in text:
        for match in re.findall(r"https?://[^\\s\"']+", text):
            if "127.0.0.1" not in match and "localhost" not in match:
                findings.append({"kind": "external-url", "match": "blocked-product-home-url"})
                break
    if report_path:
        report = Path(report_path)
        if report.exists():
            ...
            findings.extend(_scan_sandbox_sensitive_text(report_text))
            if any(term in report_text for term in blocked_terms + runtime_terms):
                findings.append({"kind": "sensitive-pattern", "match": "blocked-product-home-report"})
    return {"safe": not findings, "findings": findings}
...
def scan_local_run_doctor_outputs(payload: dict | list | str, report_path: str | Path | None = None) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = _scan_sandbox_sensitive_text(text)
    blocked_terms = [
        "api_key=demo",
    ...
    for term in blocked_terms + runtime_terms:
        if term in text:
            findings.append({"kind": "sensitive-pattern", "match": "blocked-local-run-doctor-output"})
            break
    if "http://" in text or "https://" in text:
        for match in re.findall(r"https?://[^\\s\"']+", text):
            if "127.0.0.1" not in match and "localhost" not in match:
                findings.append({"kind": "external-url", "match": "blocked-local-run-doctor-url"})
                break
    if report_path:
        report = Path(report_path)
        if report.exists():
            ...
            findings.extend(_scan_sandbox_sensitive_text(report_text))
            if any(term in report_text for term in blocked_terms + runtime_terms):
                findings.append({"kind": "sensitive-pattern", "match": "blocked-local-run-doctor-report"})
    return {"safe": not findings, "findings": findings}


def scan_guided_setup_outputs(payload: dict | list | str, report_path: str | Path | None = None) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = _scan_sandbox_sensitive_text(text)
    blocked_terms = [
        "api_key=demo",
    ...
    for term in blocked_terms + runtime_terms:
        if term in text:
            findings.append({"kind": "sensitive-pattern", "match": "blocked-guided-setup-output"})
            break
    if "http://" in text or "https://" in text:
        for match in re.findall(r"https?://[^\\s\"']+", text):
            if "127.0.0.1" not in match and "localhost" not in match:
                findings.append({"kind": "external-url", "match": "blocked-guided-setup-url"})
                break
    if report_path:
        report = Path(report_path)
        if report.exists():
            ...
            findings.extend(_scan_sandbox_sensitive_text(report_text))
            if any(term in report_text for term in blocked_terms + runtime_terms):
                findings.append({"kind": "sensitive-pattern", "match": "blocked-guided-setup-report"})
    return {"safe": not findings, "findings": findings}
...
def scan_backtest_dashboard_outputs(payload: dict | list | str, report_path: str | Path | None = None) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings = _scan_sandbox_sensitive_text(text)
    blocked_terms = [
        "api_key=demo",
    ...
    for term in blocked_terms:
        if term in text:
            findings.append({"kind": "sensitive-pattern", "match": "blocked-backtest-dashboard-output"})
            break
    if "http://" in text or "https://" in text:
        for match in re.findall(r"https?://[^\\s\"']+", text):
            if "127.0.0.1" not in match and "localhost" not in match:
                findings.append({"kind": "external-url", "match": "blocked-backtest-dashboard-url"})
                break
    if report_path:
        report = Path(report_path)
        if report.exists():
            ...
            findings.extend(_scan_sandbox_sensitive_text(report_text))
            if any(term in report_text for term in blocked_terms):
                findings.append({"kind": "sensitive-pattern", "match": "blocked-backtest-dashboard-report"})
    return {"safe": not findings, "findings": findings}
