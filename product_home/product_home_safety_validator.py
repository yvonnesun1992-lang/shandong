...
def validate_product_home_safety(payload: object) -> dict:
    text = json.dumps(payload, default=str).lower() if not isinstance(payload, str) else payload.lower()
    findings: list[dict] = []
    if isinstance(payload, dict):
        if payload.get("product_home_only") is False:
            findings.append({"kind": "boundary", "match": "product_home_only_false"})
        if payload.get("dashboard_read_only") is False:
            findings.append({"kind": "boundary", "match": "dashboard_read_only_false"})
...
def build_product_home_safety_summary() -> dict:
    status = get_product_home_status()
    safety = validate_product_home_safety(status)
