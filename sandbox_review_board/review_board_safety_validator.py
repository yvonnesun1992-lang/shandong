...
def validate_review_board_safety(payload: dict | list | str | None = None) -> dict:
    payload = payload or {}
    findings: list[str] = []
    if isinstance(payload, dict):
        for field in BLOCKED_TRUE_FIELDS:
            if payload.get(field) is True:
                findings.append(f"{field} must remain false in V5.30")
    ...
    return {
        **boundary(),
        "safe": not findings,
        "findings": findings,
        "warnings": [] if not findings else ["review board safety boundary violation detected"],
    }


def build_review_board_safety_summary() -> dict:
    return {
        **boundary(),
        "safe": True,
        "checks": [
        ...
        "warnings": ["review board decision remains NO_GO in V5.30"],
    }
