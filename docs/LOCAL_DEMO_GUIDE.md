...
4. Start the API with `python -m uvicorn src.api.v2.server:create_v2_api_app --factory --reload` if uvicorn is available.
5. Open or describe the Admin Console at `/api/v2/admin/console` and the frontend page at `web/frontend/app/admin/page.tsx`.
6. Explain safety boundaries: no broker connection, no automatic trading, no real payment execution, no external AI calls, and no production secrets.
