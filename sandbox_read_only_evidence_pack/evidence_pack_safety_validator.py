...
def validate_evidence_pack_safety(payload: dict | list | str | None = None) -> dict:
    payload = payload or {}
    findings: list[str] = []
    if isinstance(payload, dict):
        for field in BLOCKED_TRUE_FIELDS:
            if payload.get(field) is True:
                findings.append(f"{field} must remain false in V5.37")
        for key in UNREDACTED_VALUE_KEYS:
            if isinstance(payload.get(key), Number):
                findings.append(f"{key} must not contain numeric values in V5.37")
    ...
    return {
        **boundary(),
        "safe": not findings,
        "findings": findings,
        "warnings": [] if not findings else ["read-only evidence pack safety boundary violation detected"],
    }


def build_evidence_pack_safety_summary() -> dict:
    return {
        **boundary(),
        "safe": True,
        "checks": [
            "evidence pack runtime disabled",
            "evidence pack pass disabled",
        ...
        "warnings": ["read-only evidence pack remains evidence-only in V5.37"],
    }
