...
def validate_soak_safety(payload: dict | list | str) -> dict:
    ...
    for key in [
        ...
        if failed:
            errors.append(f"{key} must remain false in V5.25")
...
def build_soak_safety_summary() -> dict:
    return validate_soak_safety(boundary())
