...
def _no_risk_values() -> tuple[bool, str]:
    scan_files = [
        ...
        PROJECT_ROOT / "docs" / "V4_PRODUCTION_LAUNCH_READINESS.md",
    ]
...
def run_production_launch_readiness_check() -> dict:
    original_auth_mode = os.environ.get("SHANDONG_AUTH_MODE")
    try:
        os.environ["SHANDONG_AUTH_MODE"] = "local"
        checks = [
            _check("v3_release_candidate_check", lambda: _call_check_module("scripts.v3_release_candidate_check", "run_v3_release_candidate_check")),
            ...
            _check("v3_release_candidate_endpoint", lambda: _api_endpoint("/api/v2/system/v3-release-candidate")),
            _check("onboarding_endpoint", lambda: _api_endpoint("/api/v2/system/onboarding")),
            _check("workspace_demo_endpoint", lambda: _api_endpoint("/api/v2/system/workspace-demo")),
            _check("pricing_plan_endpoint", lambda: _api_endpoint("/api/v2/system/pricing-plan")),
            _check("admin_console_local", _admin_console_local),
            _check("v3_product_demo_freeze_doc", lambda: _exists("docs/V3_PRODUCT_DEMO_FREEZE.md")),
            _check("product_onboarding_doc", lambda: _exists("docs/PRODUCT_ONBOARDING.md")),
            _check("workspace_demo_doc", lambda: _exists("docs/WORKSPACE_DEMO_FLOW.md")),
            _check("commercial_readiness_doc", lambda: _exists("docs/COMMERCIAL_READINESS.md")),
            _check("external_deployment_doc", lambda: _exists("docs/EXTERNAL_DEPLOYMENT_DRY_RUN.md")),
            _check("production_identity_plan_doc", lambda: _exists("docs/PRODUCTION_IDENTITY_PLAN.md")),
            _check("observability_plan_doc", lambda: _exists("docs/OBSERVABILITY_PLAN.md")),
            _check("readme", lambda: _exists("README.md")),
            _check("review_package", lambda: _exists("REVIEW_PACKAGE.md")),
